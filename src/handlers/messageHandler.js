...
class MessageHandler {
  constructor(redis, logger) {
    ...
    this.vipManager = new VIPManager(redis, logger);
  ...
  async handleIntent(sock, jid, phone, intent, originalMessage) {
    try {
      // Usar o menu.js corrigido para processar intenções
      const menuHandler = require('../lib/menu');
      const response = await menuHandler.handleIncoming({
        from: jid,
      ...
      if (response) {
        await this.sendResponse(sock, jid, response);
      }
    } catch (error) {
      this.logger.error('Error in handleIntent:', error);
  ...
  async handleVipEntry(sock, jid, phone, text) {
    try {
      const result = await this.vipManager.processVIPEntry(phone, text);
      await this.sendResponse(sock, jid, result.message);
      
      if (result.success) {
        this.logger.info(`VIP entry processed for ${phone}: ${result.data.id}`);
      } else {
        this.logger.warn(`VIP entry failed for ${phone}: ${result.message}`);
      }
    } catch (error) {
      const errorResult = this.errorHandler.handleError(error, 'vip_entry', { phone, text });
      await this.sendResponse(sock, jid, '❌ Erro ao processar entrada VIP. Tente novamente.');
      this.logger.error('Error processing VIP entry:', errorResult);
    }
  }
