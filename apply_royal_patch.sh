...
mkdir -p "$WB/storage/royal"
mkdir -p "$WB/config"

if grep -q '^CLUB_NAME=' "$WB/.env" 2>/dev/null; then
  sed -i 's/^CLUB_NAME=.*/CLUB_NAME=Royal Club/' "$WB/.env"
else
  echo "CLUB_NAME=Royal Club" >> "$WB/.env"
fi

cat > "$WB/config/products.json" <<'JSON'
{
  "items": [
    { "slug": "ingresso-unissex", "name": "Ingresso Unissex", "price": 60.00 }
  ]
}
JSON

cat > "$WB/config/menus.json" <<'JSON'
{
  "menu": [
    "🎤 *Royal Club* — MC Daniel",
    "📅 16 AGO — SÁB » 22h",
    "1) Ingressos",
    "2) Mapa/Camarotes",
    "3) Lista/VIP",
...
cat > "$WB/src/lib/menu.js" <<'JS'
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { buildPixLink } from './pix.js';
import { saveVipEntry } from './store.js';
...
export async function handleIncoming({ from, text, sock }){
  const i = intent(text);
  ...
  switch(i){
    ...
    default:
      if (/^vip\s+/i.test(text)){
        await saveVipEntry({ from, payload: text });
...
echo "[✓] Patch Royal Club aplicado. Ajuste/coloque as imagens em wa-bot/storage/royal/ e rode o compose."
